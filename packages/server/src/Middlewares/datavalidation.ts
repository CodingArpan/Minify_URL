...
export default class validation {
    ...
    static shorturlrequest = (req: Request, res: Response, next: NextFunction): void => {
        let schema = yup.object().shape({
            destination: yup.string().min(13, 'URL must be atleast 13 characters long').url().required(),
            custom: yup.boolean().required(),
            keyword: yup.string().when('custom', {
                is: true,
                then: yup.string().min(4, 'keyword must be atleast 4 characters long').required(),
                otherwise: yup.string().max(0),
            }),
            secure: yup.boolean().required(),
            password: yup.string().when('secure', {
                is: true,
                then: yup.string().min(6, 'password must be atleast 6 characters long and contains only alphabets and numbers').matches(/[a-zA-Z0-9]{6,}/g, 'password must be atleast 6 characters long and contains only alphabets and numbers').required(),
                otherwise: yup.string().max(0),
            }),
            login: yup.boolean().required(),
